...
class DiaAgenda {
  ...
  factory DiaAgenda.fromJson(Map<String, dynamic> json) {
    return DiaAgenda(
      fecha:        json['fecha'],
...
class JornadaInfo {
  ...
  factory JornadaInfo.fromJson(Map<String, dynamic> json) {
    return JornadaInfo(
      id:          json['id'] ?? '',
...
class CitaCard {
  ...
  factory CitaCard.fromJson(Map<String, dynamic> json) {
    return CitaCard(
      id:            json['_id'] ?? '',
...
class BarberAppointmentsScreen extends StatefulWidget {
  ...
  const BarberAppointmentsScreen({super.key, this.barberId, this.barberName});
...
class _BarberAppointmentsScreenState extends State<BarberAppointmentsScreen> {
  ...
  Future<void> _cargarEstado() async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        setState(() => _workMode = data['workMode'] ?? 'offline');
      }
    } catch (e) {
  ...
  Future<void> _cargarSemana() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        final data = json.decode(res.body);
        final lista = (data['semana'] as List)
            .map((d) => DiaAgenda.fromJson(d))
            .toList();
        setState(() {
          _semana    = lista;
          _isLoading = false;
                ...
                children: [
                    ...
                    child: _buildTimePicker(
                      ...
                      onTap: () async {
                        final t = await showTimePicker(
                          context: context, initialTime: inicioHora,
                  ...
                  Expanded(
                    child: _buildTimePicker(
                      ...
                      onTap: () async {
                        final t = await showTimePicker(
                          context: context, initialTime: finHora,
                          builder: (ctx, child) => _timePickerTheme(ctx, child),
                        );
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: tieneDescanso ? _kRojo.withOpacity(0.3) : Colors.grey.shade200,
                    ),
                  ),
                  child: Row(
                    children: [
                  ...
                  children: [
                    Expanded(
                      child: _buildTimePicker(
                        ...
                        onTap: () async {
                          final t = await showTimePicker(
                            context: context, initialTime: descansoInicio,
                            builder: (ctx, child) => _timePickerTheme(ctx, child),
                          );
                    ...
                    Expanded(
                      child: _buildTimePicker(
                        ...
                        onTap: () async {
                          final t = await showTimePicker(
                            context: context, initialTime: descansoFin,
                            builder: (ctx, child) => _timePickerTheme(ctx, child),
                          );
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: _kRojo, elevation: 0,
  ...
  Widget _buildModeBadge() {
    ...
    String texto;
    switch (_workMode) {
      case 'runner':  dotColor = const Color(0xFF4ADE80); texto = 'Runner';  break;
      case 'agenda':  dotColor = const Color(0xFF4ADE80); texto = 'Agenda';  break;
      case 'hibrido': dotColor = const Color(0xFFFB923C); texto = 'Híbrido'; break;
      default:        dotColor = _kBlanco.withOpacity(0.4); texto = 'Offline';
    }
    return Container(
                ...
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      color: dia.tieneJornada
                          ? (selec ? _kBlanco : _kRojo)
                          : Colors.transparent,
                    ),
                  ),
                ],
  ...
  Widget _buildJornadaStat(String valor, String label) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Text(valor,
            style: const TextStyle(color: _kBlanco, fontWeight: FontWeight.bold, fontSize: 14)),
  ...
  Widget _buildListaCitas() {
    ...
    return Column(
      children: _citas.map((cita) => _buildCitaCard(cita)).toList(),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  image: cita.clienteFoto != null
                      ? DecorationImage(
                          image: NetworkImage('$baseUrl${cita.clienteFoto}'),
                          fit: BoxFit.cover)
                      : null,
                ),
          ...
          const SizedBox(height: 16),
          if (cita.status == 'aceptada' || cita.status == 'en_camino')
            SizedBox(
              width: double.infinity,
              child: ElevatedButton.icon(
                style: ElevatedButton.styleFrom(
                  backgroundColor: cita.status == 'en_camino' ? Colors.orange : _kRojo,
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
                ...
                label: Text(
                  cita.status == 'en_camino' ? 'Navegando...' : 'IR AL DOMICILIO',
  ...
  Widget _buildCitaInfo(IconData icon, String text) {
    return Row(
      ...
      children: [
        ...
        const SizedBox(width: 4),
        Text(text,
          style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: _kNegro)),
      ],
    );
  }
