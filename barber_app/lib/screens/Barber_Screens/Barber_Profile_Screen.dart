...
class _BarberProfileScreenState extends State<BarberProfileScreen> {
  ...
  bool get _esMotorizado =>
      _vehicleType == 'Auto' || _vehicleType == 'Motocicleta';
  ...
  Future<void> _cargarDatos() async {
    ...
    try {
      // 1. Datos personales del barbero
      final barberRes = await http.get(
        Uri.parse('$baseUrl/api/auth/barber/${widget.barberId}'),
      );
      ...
      // 3. Status de campos faltantes
      final statusRes = await http.get(
        Uri.parse('$baseUrl/api/upload/barber-profile-status/${widget.barberId}'),
      );
      if (statusRes.statusCode == 200) {
        final data = json.decode(statusRes.body);
        setState(() {
          _camposFaltantes = List<String>.from(data['camposFaltantes'] ?? []);
        });
      }
  ...
  Future<void> _guardar() async {
    setState(() => _isLoading = true);
    try {
      // 1. Actualizar datos personales si cambiaron
      ...
      // 2. Subir documentos y datos del vehículo
      ...
      if (response.statusCode == 200) {
        await _cargarDatos(); // Recarga para actualizar status
        if (!mounted) return;

        if (_puedeIrAlHome) {
          // Perfil completo → ir al Home
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(
              builder: (_) => BarberHomeScreen(
                barberId:   widget.barberId,
                barberName: _nombreController.text.trim(),
              ),
            ),
          );
        } else {
          ...
          child: Column(
            ...
            children: [
              ...
              _buildPasswordField('Contraseña actual', actualCtrl, verActual,
                () => setModalState(() => verActual = !verActual)),
              const SizedBox(height: 12),
              _buildPasswordField('Nueva contraseña', nuevaCtrl, verNueva,
                () => setModalState(() => verNueva = !verNueva)),
              const SizedBox(height: 12),
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () async {
                    ...
                    if (nuevaCtrl.text.length < 6) {
                      _mostrarSnack('La contraseña debe tener al menos 6 caracteres');
                      return;
                    }
  ...
  void _mostrarEditarNombre() {
    ...
    if (_cambiosNombre >= 2 && _ultimoCambioNombre != null) {
      final diasRestantes = 180 -
          DateTime.now().difference(_ultimoCambioNombre!).inDays;
      if (diasRestantes > 0) {
        _mostrarSnack('Puedes cambiar tu nombre en $diasRestantes días');
        return;
      }
    }
      ...
      builder: (_) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Puedes cambiar tu nombre ${2 - _cambiosNombre} vez más en los próximos 6 meses.',
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  setState(() => _nombreController.text = ctrl.text.trim());
                  Navigator.pop(context);
                },
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'Completa los campos marcados con * para usar la app.',
  ...
  Widget _buildCardPersonal() {
    return _buildCard([
      ...
      _buildFilaEditable(
        label: 'Teléfono',
        valor: _telefonoController.text.isNotEmpty
            ? _telefonoController.text
            : 'Sin teléfono',
        onTap: () => _mostrarEditarCampo(
          'Teléfono', _telefonoController, Icons.phone_outlined),
      ),
      _buildFilaInfo(
            ...
            child: Column(
              children: [
                      ...
                      return GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.circular(12),
                            border: Border.all(
                              color: selec ? _kRojo : Colors.grey.shade200,
                              width: selec ? 1.5 : 0.5,
                            ),
                          ),
                          child: Center(
                ...
                if (_vehicleType != null) ...[
                  ...
                  // Color — siempre visible para todos los vehículos
                  _buildFilaEditable(
                    label:      'Color *',
                    valor:      _colorController.text.isNotEmpty
                        ? _colorController.text : 'Sin completar',
                    esFaltante: _camposFaltantes.contains('vehicleColor'),
                    onTap: () => _mostrarEditarCampo(
                      'Color', _colorController, Icons.color_lens_outlined),
                  ),

                  // Año — siempre visible para todos los vehículos
                  _buildFilaEditable(
                    label: 'Año',
                    valor: _yearController.text.isNotEmpty
                        ? _yearController.text : 'Sin completar',
                    onTap: () => _mostrarEditarCampo(
                      'Año', _yearController, Icons.calendar_today_outlined,
                      tipo: TextInputType.number),
                  ),

                  // Solo para Auto y Motocicleta
                  if (_esMotorizado) ...[
                    _buildFilaEditable(
                      label:      'Marca *',
                      valor:      _brandController.text.isNotEmpty
                          ? _brandController.text : 'Sin completar',
                      esFaltante: _camposFaltantes.contains('vehicleBrand'),
                      onTap: () => _mostrarEditarCampo(
                        'Marca del vehículo', _brandController,
                        Icons.branding_watermark_outlined),
                    ),
                    _buildFilaEditable(
                      label:      'Modelo *',
                      valor:      _modelController.text.isNotEmpty
                          ? _modelController.text : 'Sin completar',
                      esFaltante: _camposFaltantes.contains('vehicleModel'),
                      onTap: () => _mostrarEditarCampo(
                        'Modelo', _modelController, Icons.directions_car_outlined),
                    ),
                    _buildFilaEditable(
                      label:      'Placas *',
                      valor:      _plateController.text.isNotEmpty
                          ? _plateController.text : 'Sin completar',
                      esFaltante: _camposFaltantes.contains('vehiclePlate'),
                      onTap: () => _mostrarEditarCampo(
                        'Placas', _plateController, Icons.pin_outlined),
                    ),
                  ],
                ],
              ],
  ...
  Widget _buildDocumentos() {
    final docs = [
      _DocItem('Foto de perfil *', 'profileImage', _profileImage, _profileImageUrl,
        (f) => setState(() => _profileImage = f)),
      _DocItem('INE frente *', 'ineFrente', _ineFrente, _ineFrenteUrl,
        (f) => setState(() => _ineFrente = f)),
      _DocItem('INE reverso', 'ineReverso', _ineReverso, _ineReversoUrl,
        (f) => setState(() => _ineReverso = f)),
      if (_esMotorizado) ...[
        _DocItem('Licencia *', 'licenseImage', _licenseImage, _licenseImageUrl,
          (f) => setState(() => _licenseImage = f)),
        _DocItem('Foto vehículo', 'vehiclePhoto', _vehiclePhoto, _vehiclePhotoUrl,
          (f) => setState(() => _vehiclePhoto = f)),
        _DocItem('Foto placas', 'platePhoto', _platePhoto, _platePhotoUrl,
          (f) => setState(() => _platePhoto = f)),
      ],
    ];
  ...
  Widget _buildDocBox(_DocItem doc) {
    ...
    } else if (doc.networkUrl != null) {
      image = NetworkImage('$baseUrl${doc.networkUrl}');
    }
      ...
      child: Container(
        ...
        child: image == null
            ? Column(
                ...
                children: [
                  Icon(
                    esFaltante ? Icons.add_photo_alternate : Icons.add_photo_alternate_outlined,
                    color: esFaltante ? _kRojo : Colors.grey.shade400,
                    size: 26,
                  ),
            ...
            : Align(
                ...
                child: Container(
                  ...
                  child: Text(doc.label,
                    style: const TextStyle(color: _kBlanco, fontSize: 10)),
                ),
              ),
      ),
    );
    ...
    return Padding(
      ...
      child: SizedBox(
        ...
        child: OutlinedButton(
          ...
          child: const Text('Cambiar contraseña',
            style: TextStyle(color: _kNegro, fontWeight: FontWeight.w600, fontSize: 15)),
        ),
      ),
    );
  }
                ...
                children: [
                  ...
                  Text(valor,
                    style: TextStyle(
                      fontSize: 14,
                      fontWeight: FontWeight.w600,
                      color: esFaltante ? _kRojo : _kNegro,
                    )),
                ],
              ),
            ),
              ...
              children: [
                ...
                Text(valor,
                  style: TextStyle(
                    ...
                    color: muted ? Colors.grey.shade400 : _kNegro,
                  )),
              ],
            ),
          ),
    ...
    return TextField(
      ...
      decoration: InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(12),
          borderSide: const BorderSide(color: _kRojo, width: 1.5),
        ),
      ),
    );
  }
            ...
            children: [
              ...
              ListTile(
                leading: Container(
                  ...
                  child: const Icon(Icons.camera_alt_outlined, color: _kRojo, size: 20)),
              ...
              ListTile(
                leading: Container(
                  width: 40, height: 40,
                  decoration: BoxDecoration(color: _kRojo.withOpacity(0.08), borderRadius: BorderRadius.circular(10)),
                  child: const Icon(Icons.photo_library_outlined, color: _kRojo, size: 20)),
